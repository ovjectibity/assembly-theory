...
use crate::plugin::rubiks::{CornerSlot, CubeMove, Cubelet, FaceColor, RubiksCube, SlottedCubelet};
...
impl RubiksSolver {
    ...
    fn init_policy() -> (nn::Sequential,nn::VarStore) {
    ...
    fn get_reward(cube_current: &RubiksCube, cube_previous: &RubiksCube) -> f32 {
        Self::get_score(cube_current) - Self::get_score(cube_previous)
    }
    ...
    fn index_cube_move(index: u32) -> Option<CubeMove> {
        match index {
    ...
    //Output tensor is [num_trajectory,1]
    fn log_probs_policy(logits: &Tensor,mv: &Tensor) -> Tensor {
        // logits.get(Self::get_cube_move_index(&mv)).log()
        logits.gather(2,&mv.unsqueeze(2),false).log().sum(tch::Kind::Float)
    }
    ...
    fn policy_loss(log_probs: Tensor, rewards: Tensor) -> Tensor {
        - (log_probs * rewards).mean(tch::Kind::Float)
    }
    ...
    pub fn train_an_epoch(&mut self,trajs: Vec<(Vec<CubeMove>,Vec<RubiksCube>,Vec<f32>)>) {
        ...
        let loss = Self::policy_loss(Self::log_probs_policy(&logits, &mv_t), rewards_t);
